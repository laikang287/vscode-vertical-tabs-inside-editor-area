...
- 在扩展操作与 VS Code 横向标签操作之间增加完整分割线和两行兼容性说明，并在悬停信息中标明关闭原生操作区的配置项。
- 原生操作说明支持全部 10 种界面语言和无障碍分组描述；窄栏下限制菜单宽度、换行说明并省略过长命令名称。
- 保持原生菜单按需异步加载：关闭 `verticalTabs.showNativeContextMenuActions` 或没有可用操作时不显示分割线、说明和第二组菜单。
