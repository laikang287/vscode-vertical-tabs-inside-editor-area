...
suite('Vertical Tabs extension', () => {
  ...
  test('corrects only the vertical-tabs group after a third editor group expands', async function () {
    ...
    await vscode.commands.executeCommand('vscode.setEditorLayout', {
      orientation: 0,
      // Request the rail below VS Code's native minimum so the scenario stays
      // deterministic when window chrome changes the available editor width.
      groups: [{ size: 180 }, { size: 1200 }, { size: 220 }],
    });
    await waitForEditorLayout((candidate) => (
