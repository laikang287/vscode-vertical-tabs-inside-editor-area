...
export class VerticalTabsPanel {
  ...
  private async repairExclusiveRail(source: string): Promise<boolean> {
    ...
    for (let attempt = 0; attempt < GROUP_PUBLISH_WAIT_ATTEMPTS; attempt += 1) {
      ...
      const destinationHasMovedTab = vscode.window.tabGroups.all.includes(destination)
        && destination.tabs.some((tab) => sameIdentity(targetIdentity(tab), movedIdentity));
      if (unexpectedCountAfter >= unexpectedCountBefore || !destinationHasMovedTab) {
