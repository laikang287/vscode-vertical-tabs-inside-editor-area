...
- 只有当前 Git 分支为 `main` 时，才允许按本节规则自动新增、修改或更新根目录下的 `todo_list.csv`；其它分支不得自动更新该文件，除非用户明确要求。
- 维护根目录下的 `todo_list.csv`。登记以对话窗口为单位：每个新对话中首次出现涉及代码改动的用户请求时，必须登记一条；同一对话窗口最多登记一次。
- `todo_list.csv` 必须统一使用 UTF-8 编码（无 BOM）；后续发现该文件变为其它编码时，必须先转换回 UTF-8，转换时不得改变记录内容、列结构或原有行尾格式。
- 同一对话窗口后续无论用户追加、调整或要求实现其它代码改动，都不得再次登记或新增记录；功能完成后仍应按本节规则更新首次登记记录的解决状态。
- 纯咨询、只读检查或审查，以及仅修改协作规则或文档且不涉及代码改动的请求，不触发登记；若同一对话后续首次提出代码改动，再于该时登记一次。
